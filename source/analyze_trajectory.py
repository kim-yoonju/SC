...
def plot(stats: list[dict], input_path: str, out_path: str | None):
    ...
    fig.suptitle(f"{Path(input_path).name}  (n={n})", fontsize=14, fontweight="bold", y=0.98)
    ...
    for at in autotexts3:
        at.set_fontsize(8)
    ax3.legend(wedges3, [f"{c}  ({v})" for c, v in zip(cat_labels, cat_sizes)],
               loc="lower center", bbox_to_anchor=(0.5, -0.18), fontsize=8, ncol=2)
    ax3.set_title("trajectory category", fontsize=12, fontweight="bold")
    ...
    ax5.bar(step_xlbls, step_ys, color="#4CAF50", edgecolor="white", linewidth=0.5)
