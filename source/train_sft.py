...
def train(args):
    ...
    # --action_weights: Next action 스페셜 토큰의 역빈도 가중치 (solve=1.0 기준)
    action_weight_map = None
    if args.action_weights:
        action_weight_map = compute_action_weights(args.data_path, tokenizer)
        if is_main and not action_weight_map:
            print("[Auto] Next action 가중치 미적용 (action token 없음)")

    # --rubric_weights: 루브릭 토큰 위치에 focal loss 적용 (γ=focal_gamma)
    rubric_weight_map = None
    if args.rubric_weights:
        rubric_weight_map = compute_rubric_weights(args.data_path, tokenizer, max_weight=1.0)
        if is_main:
            if rubric_weight_map:
                print(f"[rubric_focal] Fail rubrics 토큰 위치에 focal loss 적용 (γ={args.focal_gamma})")
            else:
                print("[rubric_focal] rubric token 없음 — focal loss 미적용")

    dataset   = PreprocessedDataset(args.data_path, tokenizer,
                                    max_length=args.max_length,
                                    skip_error=args.skip_error,
                                    action_weight_map=action_weight_map,
                                    rubric_weight_map=rubric_weight_map)
...
def parse_args():
    ...
    p.add_argument("--action_weights", action="store_true", default=False,
                   help="Next action 스페셜 토큰에 데이터 역빈도 가중치 적용 (solve 기준 자동 계산)")
