...
def _load_rubric_str_simple() -> str:
    ...
    with open(_rubric_file, encoding="utf-8") as f:
        rubrics = json.load(f)
    return "\n".join(f"{i}. {r['name']}: {r.get('simple_criterion', r['criterion'][:80])}"
                     for i, r in enumerate(rubrics, 1))
...
def build_sft_sample_classification(
    ...
    return {
        ...
        "target":    target,
        "inference": step.get("inference", ""),
        "is_error":  False,   # classification target은 항상 critic — masking 불필요
...
def preprocess_mode(
    ...
    for item in raw:
        ...
        for k, step in enumerate(steps):
            if mode == "inference" and step.get("is_error", False):
                continue
            if mode == "classification":
                # gold_fail_rubrics=None은 유효 — "페일 없음" → <|none|> 로 처리
                _cls_missing = [f for f in ("prm_fast_critique", "prm_deep_critique")
...
def debug(preprocessed_path: str, idx: int = 1):
    """저장된 전처리 JSONL에서 is_error=False인 샘플 중 idx번째(0-based)를 읽어 TARGET을 출력한다."""
    sep = "─" * 72
    with open(preprocessed_path, encoding="utf-8") as f:
        ...
        for line in f:
            ...
            if sample.get("is_error", False):
                continue
            if count == idx:
                break
            count += 1
        else:
            print(f"is_error=False 샘플 {idx}번을 찾을 수 없습니다.")
            return
    print(f"\n{'='*72}")
...
def preprocess(
    ...
    is_error=False 스텝 → 전체 시퀀스에 loss (좋은 inference 학습)
    is_error=True  스텝 → inference 마스킹 후 Fast critic~Next action만 loss
