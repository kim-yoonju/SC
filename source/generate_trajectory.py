...
--num_start 0 \
--num_end 500 \
--data_path /mnt/yoonju/SC/datasets/deepmath_16k/base_inference_wrong.jsonl \
--resume_folder /mnt/yoonju/SC/output/sft_trajectory/20260525_024833_4
...
스텝 state / next_gold_action:
  state = {gen|pat}_{solve|rethink|end}
  prefix  gen : source=gen (rethink 모드 포함)
          pat : source=patcher
  suffix  end     : boxed{} 답 출력 스텝
          rethink : 이전 스텝 next_gold_action=<|rethink|>
          solve   : 그 외 (첫 스텝 포함)
"""
...
from utils import (
    CONF,
    TOKEN_SOLVE, TOKEN_CORRECT, TOKEN_END, TOKEN_NONE,
...
def _load_action_prompts() -> dict[str, str]:
    ...
    _rubric_rel = _prm_sect.get("deep_rubric")
    if not _rubric_rel:
        raise KeyError("config.PRM.deep_rubric 설정이 없습니다")
    rubric_file = Path(_rubric_rel) if Path(_rubric_rel).is_absolute() else _ROOT_PATH / _rubric_rel

    full_lines   = []   # SFT 모델용: criterion 풀 텍스트
    simple_lines = []   # base 모델용: simple_criterion 한 줄
    ...
    # base 모델용 Fail rubrics 섹션: 토큰 없이 루브릭 이름만
    _FAIL_RUBRICS_SIMPLE = (
        "Fail rubrics:\n"
        "  Write the name of each rubric still incorrect after Deep critic, one per line.\n"
        "  If all passed: none"
    )
    ...
    _NO_RUBRIC_RE = re.compile(
        r"ERROR TYPES TO AVOID[^\n]*\n\{\{rubric\}\}\n+",
    )

    prompts: dict[str, str] = {}
    for fname in ["generator.json", "prm.json", "step_manager.json", "patcher.json"]:
        with open(_PROMPTS_PATH / fname, encoding="utf-8") as f:
            for e in json.load(f):
                prompts[e["name"]] = e["content"].replace("{{rubric}}", rubric_full)
                if "{{rubric}}" in e["content"]:
                    simple = e["content"].replace("{{rubric}}", rubric_simple)
                    simple = _FAIL_RUBRICS_RE.sub(_FAIL_RUBRICS_SIMPLE, simple)
                    prompts[e["name"] + "_simple"] = simple
                    prompts[e["name"] + "_no_rubric"] = _NO_RUBRIC_RE.sub("", e["content"])
    return prompts

_ACTION_PROMPTS = _load_action_prompts()
_IS_SFT_MODEL   = False  # generate_trajectory.py는 항상 base 모델로 실행
_BASE_MODE      = False  # --base_mode 시 True: generator 프롬프트에서 루브릭 제거 (PRM은 그대로)
...
from PRM import (
    ...
    evaluate_step, load_deep_rubrics, load_fast_rubric,
)
...
def _build_rethink_explanation(state: "ProblemState") -> str:
    """rethink 프롬프트용 오류 설명 — 틀린 스텝 does + PRM critique + guidance."""
    if _BASE_MODE:
        return "The previous step was incorrect — try a completely different approach."
    ...
    fail_entries = [e for e in prm_deep if e.get("verdict") == "incorrect"]
...
def _load_atomicity_system_prompt() -> str:
    ...
    _rubric_rel = _prm_cfg.get("deep_rubric", _prompts_cfg.get("rubric_file", "prompts/deep_rubric_v6.1.json"))
...
def _load_devide_step_prompt() -> str:
    path = Path(__file__).resolve().parent.parent / "prompts" / "step_manager.json"
    if not path.exists():
        return ""
    with open(path, encoding="utf-8") as f:
        for obj in json.load(f):
            if obj.get("name") == "devide_step":
                return obj.get("content", "")
    return ""

_DEVIDE_STEP_PROMPT = _load_devide_step_prompt()
...
def _load_rethink_guidance() -> dict[str, str]:
    ...
    with open(path, encoding="utf-8") as f:
        entries = json.load(f)
    return {e["name"]: e["rethink_guidance"] for e in entries
            if e.get("name") and e.get("rethink_guidance")}
...
# vLLM 단일 GPU 모드에서 generate_batch가 초기화 후 채워주는 전역 핸들
_VLLM_LLM  = None
_VLLM_TOK  = None
_VLLM_LOCK: "threading.Lock | None" = None
...
def _decompose_with_atomicity(
    ...
    HF step_manager가 없으면 vLLM(_VLLM_LLM)으로 fallback.
    """
    ...
    elif _VLLM_LLM is not None:
        prompt = build_chat_prompt(_VLLM_TOK, _DECOMPOSE_SYSTEM, user_msg)
        with _VLLM_LOCK:
            resp = _vllm_texts(_VLLM_LLM, [prompt], 512)[0].strip()
    else:
        return None
...
def _split_step_with_devide_step(
    step_text: str,
    sm_model,
    sm_tok,
) -> tuple[str, str] | None:
    """devide_step 프롬프트로 스텝을 두 파트로 분리.
    반환: (part1, part2) 또는 None (atomic or 분리 불가)
    HF step_manager가 없으면 vLLM(_VLLM_LLM)으로 fallback.
    """
    if not _DEVIDE_STEP_PROMPT:
        return None

    if sm_model is not None:
        prompt = build_chat_prompt(sm_tok, _DEVIDE_STEP_PROMPT, f"Step:\n{step_text}")
        with _DECOMPOSE_LOCK:
            inputs = sm_tok(prompt, return_tensors="pt").to(sm_model.device)
            with torch.no_grad():
                out = sm_model.generate(
        ...
        prompt = build_chat_prompt(_VLLM_TOK, _DEVIDE_STEP_PROMPT, f"Step:\n{step_text}")
        with _VLLM_LOCK:
            resp = _vllm_texts(_VLLM_LLM, [prompt], 256)[0].strip()
    else:
        return None

    if resp.upper().startswith("ATOMIC"):
        return None

    m = re.match(r"SPLIT:\s*(.+)", resp, re.DOTALL)
    if not m:
        return None

    boundary = m.group(1).strip()
    idx = step_text.find(boundary)
    if idx == -1:
        logger.warning(f"[devide_step] boundary not found in step: {boundary[:80]!r}")
        return None

    split_end = idx + len(boundary)
    part1 = step_text[:split_end].strip()
    part2 = step_text[split_end:].strip()
...
def _batch_run_generator_vllm(
    ...
    for state in states:
        ...
        _hist_items = _build_gen_history_items(state.history)
        if state.is_rethink:
            ...
            if _hist_items:
                ...
                kept = []
                for item in reversed(_hist_items):
                    _chunk = tokenizer.encode(item[0])
                    if _budget - len(_chunk) < 0:
                        break
                    kept.insert(0, item)
                    _budget -= len(_chunk)
                if len(kept) < len(_hist_items):
                    logger.warning(
                        f"[history truncation] state id={state.item.get('id','?')}: "
                        f"{len(_hist_items) - len(kept)}개 오래된 스텝 잘림 "
                        f"({len(_hist_items)} → {len(kept)})"
                    )
                lines = [f"[Problem]\n{state.item['problem']}", hist_header]
                for i, (text, is_full) in enumerate(kept, 1):
                    if is_full:
                        lines.append(f"\nStep {i}:\n{text}")
                    else:
                        lines.append(f"\nStep {i}: {text}")
    ...
    _fallback_idxs = [
        ...
        if not (isinstance(out.outputs[0].stop_reason, int)
                and out.outputs[0].stop_reason in _tok_to_action)
    ]
    _forced_action: dict[int, str] = {}
    if _fallback_idxs:
        _force_params = SamplingParams(
            max_tokens=1,
            temperature=0,
            allowed_token_ids=list(_action_ids),
        )
        _force_prompts = [
            prompts[i] + outputs[i].outputs[0].text
            for i in _fallback_idxs
        ]
        _force_outputs = llm.generate(_force_prompts, _force_params, use_tqdm=False)
        for idx, fout in zip(_fallback_idxs, _force_outputs):
            gen_tids = fout.outputs[0].token_ids
            if gen_tids and gen_tids[0] in _tok_to_action:
                _forced_action[idx] = _tok_to_action[gen_tids[0]]
    ...
    for i, (output, state) in enumerate(zip(outputs, states)):
        ...
        if isinstance(stop_reason, int) and stop_reason in _tok_to_action:
            ...
            full_text += pred_action  # stop token을 text에 보존
        elif i in _forced_action:
            pred_action = _forced_action[i]
            full_text += pred_action  # 강제 파싱된 action token을 text에 보존
        else:
            # forced inference도 실패 (action_ids 없는 경우 등) — solve로 fallback
            pred_action = TOKEN_SOLVE
            logger.warning(
                f"[Generator vLLM] id={state.item.get('id')} step={step_number} "
                f"forced inference 실패 → {TOKEN_SOLVE} fallback"
            )
        ...
        is_fail        = has_tool_call or has_placeholder
        error_reason    = (
        ...
        state.pending_step = {
            ...
            "next_pred_action": pred_action.strip("<|>") if (isinstance(stop_reason, int) and stop_reason in _tok_to_action) else (_parse_next_pred_action(self_check_text) or pred_action.strip("<|>")),
            "source":           "gen",
            "role":             "rethink" if state.is_rethink else "gen",
            "was_rethink":      state.is_rethink,
            "is_fail":         is_fail,
            "is_first_pat":     state.is_rethink and not is_fail,
            "summary":          (
                {"step_analysis": f"{error_reason} — skipped PRM_log"} if is_fail else None
            ),
        }
...
def _run_patcher_api(states: list[ProblemState]) -> None:
    ...
    def _call_one(state: ProblemState) -> None:
        ...
        history_inferences = [s["inference"] for s in state.history if not s.get("is_fail")]
        ...
        logger.info(
            ...
            + (" [empty response]" if is_fail else "")
        )
        ...
        state.pending_step = {
            ...
            "full_text":        text,
            "pred_action":      pred_action,
            "next_pred_action": pred_action.strip("<|>"),
            "source":           "patcher",
            "role":             "patcher",
            "was_rethink":      True,
            "is_fail":         is_fail,
            "is_first_pat":     not is_fail,
            "summary":          (
...
def _build_prm_prev_lines(history: list[dict]) -> list[str]:
    ...
    valid = [s for s in history if not s.get("is_fail")]
    lines = []
...
def _build_gen_history_items(history: list[dict], recent_k: int = 2) -> list[tuple[str, bool]]:
    """Generator 프롬프트용 history items.
    최근 recent_k 스텝: inference 원문 (is_full=True).
    그 이전 스텝: does 요약 (is_full=False). does 없으면 inference fallback."""
    valid = [s for s in history if not s.get("is_fail")]
    n = len(valid)
    result = []
    for i, s in enumerate(valid):
        if i >= n - recent_k:
            ...
            does = (s.get("summary") or {}).get("does") or s.get("does_summary") or ""
            if does:
                result.append((does, False))
            else:
                text = s.get("inference") or s.get("text") or ""
                result.append((text, True))
    return result
...
_INFERENCE_SUMMARY_MAX_TOKENS     = 512
_INFERENCE_LONG_CHAR_THRESHOLD    = 8000   # ≈ 2048 tokens
...
def _parse_generator_self_check(text: str, rubric_names: list[str]) -> dict[str, dict]:
    ...
    _DC = r"(?:\d+\.\s*)?\*{0,2}(?:Deep\s+criti(?:que|c)|deep_critique)\*{0,2}"
    m = re.search(
        _FC + r"\s*:\*{0,2}\s*\n(.*?)(?=" + _DC + r"\s*:\*{0,2}\s*\n|$)",
...
def _parse_generator_deep_check(text: str, rubric_names: list[str]) -> dict[str, dict]:
    """Deep critic 섹션 파싱.
    지원 포맷: "Deep critique:", "**Deep critique:**", "2. **Deep critique:**", "deep_critique:"
    반환: {rubric_name: {"verdict", "critique"}}
...
def _parse_next_pred_action(text: str) -> str | None:
    ...
    'Next action:\\n solve' 또는 'Next action:\\n <|solve|>' 형태 모두 지원.
    "Next action:" 없이 텍스트 끝에 바로 액션 토큰이 붙은 경우도 fallback으로 처리."""
    m = re.search(r"Next\s+action\s*:\s*\n?\s*(?:<\|)?(solve|rethink|end)(?:\|>)?", text, re.IGNORECASE)
    if m:
        return m.group(1).lower()
    # 텍스트 끝에 <|solve|> / <|rethink|> / <|end|> 가 바로 붙은 경우
    m = re.search(r"<\|(solve|rethink|end)\|>\s*$", text, re.IGNORECASE)
    if m:
        return m.group(1).lower()
    return None
...
def _batch_run_step_summary_only(
    ...
    for state in states:
        ...
        if step.get("is_fail"):
            ...
            continue
        inference_full = step.get("inference") or step.get("text") or ""
        prompts.append(build_chat_prompt(tokenizer, _SUMMARIZE_SYSTEM,
                                         f"Step:\n{inference_full[:1200]}"))
        tasks.append(("step_summary", state))
        if len(inference_full) > _INFERENCE_LONG_CHAR_THRESHOLD:
            prompts.append(build_chat_prompt(tokenizer, _WRONG_STEP_SUMMARIZE_SYSTEM,
                                             f"Step:\n{inference_full[:3000]}"))
            tasks.append(("inference_summary", state))
        else:
            step["inference_summary"] = None
    ...
    max_new_tokens = max(
        _INFERENCE_SUMMARY_MAX_TOKENS if t == "inference_summary" else _SUMMARIZE_MAX_TOKENS
        for t, _ in tasks
    )
...
_HF_GENERATE_SUBBATCH = 16  # step_manager HF generate 최대 배치 크기 (OOM 방지)
...
def _generate_texts(model, tokenizer, device, prompts: list[str], max_new_tokens: int) -> list[str]:
    ...
    orig_side = tokenizer.padding_side
    tokenizer.padding_side = "left"
    for i in range(0, len(prompts), _HF_GENERATE_SUBBATCH):
        chunk = prompts[i : i + _HF_GENERATE_SUBBATCH]
...
def _batch_run_all_summaries_and_gen_critique(
    ...
    for state in states:
        ...
        if not step.get("is_fail"):
            inference_full = step.get("inference") or step.get("text") or ""
            step_text = inference_full[:1200]
            ...
            if len(inference_full) > _INFERENCE_LONG_CHAR_THRESHOLD:
                prompts.append(build_chat_prompt(tokenizer, _WRONG_STEP_SUMMARIZE_SYSTEM,
                                                 f"Step:\n{inference_full[:3000]}"))
                tasks.append(("inference_summary", state, None))
            else:
                step["inference_summary"] = None
            ...
            history_does = [
                (i + 1, s.get("does_summary") or s.get("does"))
                for i, s in enumerate(state.history)
                if not s.get("is_fail") and (s.get("does_summary") or s.get("does"))
            ]
            history_block = (
                "Previous steps:\n" + "\n".join(
                    f"Step {idx}: {does}" for idx, does in history_does
                ) + "\n\n"
            ) if history_does else ""
            for rn in votes:
    ...
    for state in states:
        if not state.pending_step.get("is_fail"):
            result = prm_results_map.get(id(state), {})
            votes  = result.get("votes", {})
...
def _parse_pred_fail_rubrics(
    full_text: str | None,
    rubric_names: "list[str] | None" = None,
) -> list[str]:
    """full_text의 'Fail rubrics:' 섹션에서 루브릭 토큰을 파싱.
    rubric_names 지정 시 유효성 검증으로 수식/일반 텍스트 오인식 방지."""
    if not full_text:
        return []
    m = re.search(
        r"Fail\s+rubrics\s*:\s*\n(.*?)(?=\n\s*Next\s+action|\Z)",
    ...
    token_to_name = (
        {name.lower().replace(" ", "_"): name for name in rubric_names}
        if rubric_names else None
    )
    ...
    for line in section.split("\n"):
        ...
        if not line:
            continue
        plain = re.sub(r"\\+text\{([^}]+)\}", r"\1", line).strip("\\[]$").strip()
        if plain.lower() in ("none", "none.", "n/a", ""):
            continue

        # <|rubric_token|> 형태
        tok_m = re.search(r"<\|([^|>]+)\|>", line)
        if tok_m:
            token = tok_m.group(1)
            if token_to_name is None or token in token_to_name:
                rubrics.append(token)
            continue

        # plain token name (e.g. "algebraic_manipulation" or "algebraic_manipulation:")
        candidate = re.split(r"[:\s]", line)[0].strip().lower()
        if token_to_name is not None and candidate in token_to_name:
            rubrics.append(candidate)
            continue
        if token_to_name is None and re.match(r"^[a-z][a-z0-9_]+$", candidate):
            rubrics.append(candidate)
            continue

        # 전체 루브릭 이름 직접 매치 (base model 포맷, rubric_names 필요)
        if rubric_names:
            for name in rubric_names:
                if line.lower().startswith(name.lower()):
                    rubrics.append(name.lower().replace(" ", "_"))
                    break
...
def _compute_step_action(s: dict) -> tuple[list[str], str]:
    """prm_deep_critique + is_fail 기반으로 fail_rubrics와 next_gold_action을 계산."""
    ...
    # is_fail=True면 PRM 또는 check_solved로 감지된 오류 → 반드시 rethink
    if fail_rubrics or s.get("is_fail"):
...
def _build_traj(
    ...
    for i, (s, label) in enumerate(zip(steps, labels)):
        ...
        is_wrong    = (next_action == TOKEN_CORRECT)
        is_prev_wrong = i > 0 and step_dicts[i - 1]["next_gold_action"] == TOKEN_CORRECT

        prefix = "pat" if step_src == "patcher" else "gen"
        if _has_box and not is_wrong:
            # boxed answer가 있고 실제로 correct → end
            suffix = "end"
        elif is_prev_wrong:
            # 이전 step이 wrong → rethink 시도 (has_box여도 틀렸으면 rethink)
            suffix = "rethink"
        else:
            suffix = "solve"
...
def _process_prm_result(
    ...
    def _apply_wrong(reason_suffix: str = ""):
        ...
        if TRAJ_MAX_STEPS and len(state.all_steps) >= TRAJ_MAX_STEPS:
            ...
            traj = _build_traj(problem_id, problem, gold_answer,
                               ...
                               fail_reason="max_steps", rubric_names=rubric_names)
            if save_fn:
                save_fn(traj)
            state.done = True
            return
        ...
        if state.in_substep_mode:
            ...
            if cur_depth < MAX_SUBSTEP_DEPTH and state.substep_queue:
                split = _split_step_with_devide_step(
                    step["text"],
                ...
                if split:
                    part1, part2 = split
                    sub_substeps = [
                        {"goal": part1, "depth": cur_depth + 1},
                        {"goal": part2, "depth": cur_depth + 1},
                    ]
                    state.substep_queue = sub_substeps + state.substep_queue[1:]
                    state.is_rethink    = True
                    logger.info(f"[id={problem_id}] 서브스텝 재분해 성공 depth={cur_depth+1} (devide_step)")
                    decomposed = True
            if not decomposed:
            ...
            # rethink 실패 → devide_step으로 part1만 rethink, part2는 버림
            split = _split_step_with_devide_step(
                state.last_wrong_step_text,
                step_manager_model, step_manager_tok,
            ) if state.substep_depth < MAX_SUBSTEP_DEPTH else None

            if split:
                part1, _part2 = split
                # part1만 rethink 대상으로, part2는 버림 (이후 스텝에서 자연스럽게 커버)
                state.last_wrong_does  = part1
                state.substep_depth   += 1
                # 상태 리셋: part1에 대한 rethink → split 사이클 재시작
                state.step_rethink_tried = False
                state.step_substep_tried = False
                state.is_rethink         = True
                logger.info(
                    f"[id={problem_id}] devide_step 분해(depth={state.substep_depth}): "
                    f"part1 rethink, part2 버림"
                )
            else:
                # 분리 불가(ATOMIC) or max_depth → patcher
                reason = "max_depth" if state.substep_depth >= MAX_SUBSTEP_DEPTH else "atomic"
                ...
                state.last_wrong_rubrics     = wrong_rubrics
                state.last_wrong_rubric_details = _rubric_details
                logger.info(f"[id={problem_id}] substep 분해 불가({reason}) → patcher")
            ...
            # patcher도 실패 → 종료
            _fail_reason = "gen_wrong_answer" if state.last_wrong_answer else "patcher_fail"
            print(f"  [id={problem_id}]  {_fmt(state.all_steps)}  → {_fail_reason}")
            traj = _build_traj(problem_id, problem, gold_answer,
                               state.all_steps, False, "all",
                               fail_reason=_fail_reason, rubric_names=rubric_names)
    ...
    if step.get("source") == "patcher":
        if step.get("is_fail"):
            # 빈 응답 → patcher_fail
            _fail_reason = "gen_wrong_answer" if state.last_wrong_answer else "patcher_fail"
            logger.info(f"[id={problem_id}] patcher empty response → {_fail_reason}")
            print(f"  [id={problem_id}]  {_fmt(state.all_steps)}  → {_fail_reason}")
            traj = _build_traj(problem_id, problem, gold_answer,
                               state.all_steps, False, "all",
                               fail_reason=_fail_reason, rubric_names=rubric_names)
        ...
        if has_boxed(step["text"]):
            if check_solved(step["text"], gold_answer, problem=problem):
                ...
                # patcher가 틀린 답 제출
                _fail_reason = "gen_wrong_answer" if state.last_wrong_answer else "patcher_wrong_answer"
                logger.info(f"[id={problem_id}] patcher wrong final answer → {_fail_reason}")
                state.all_steps.append(step)
                print(f"  [id={problem_id}]  {_fmt(state.all_steps)}  → {_fail_reason}")
                traj = _build_traj(problem_id, problem, gold_answer,
                                   state.all_steps, False, "all",
...
def generate_batch(
    ...
    ts_cache    = _ts(step_cache_fn)

    def _make_state(item: dict) -> ProblemState:
        state = ProblemState(item=item)
        if partial_cache:
            pid = str(item.get("id", ""))
            cached = partial_cache.get(pid)
            if cached:
                state.history       = cached.get("history", [])
                state.all_steps     = cached.get("all_steps", [])
                state.rethink_round = cached.get("rethink_round", 0)
                logger.info(f"[cache resume] id={pid}: history={len(state.history)}스텝  all_steps={len(state.all_steps)}")
        return state

    # ── PRM API: 단일 state 평가 ─────────────────────────────────────────────
    def _run_prm_one(state: ProblemState) -> dict:
        if state.pending_step.get("is_fail"):
            n = len(rubrics)
    ...
    if USE_VLLM and generators:
        # ── vLLM 모드: gen은 vLLM, sum은 step_manager(base HF) 또는 vLLM 공유 ──
        _vllm_lock = threading.Lock()
        llm_v, tok_v = generators[0][0], generators[0][1]

        global _VLLM_LLM, _VLLM_TOK, _VLLM_LOCK
        _VLLM_LLM, _VLLM_TOK, _VLLM_LOCK = llm_v, tok_v, _vllm_lock
        ...
        if step_manager_model is not None:
            # 2+ GPU: step_manager(base) HF 모델로 요약 — SFT 모델 대신 base 모델 사용
            sm_device = next(step_manager_model.parameters()).device

            def _sum_fn_sm(model, tokenizer, device, states):
            ...
            sum_worker = _GPUBatchWorker(_sum_fn_sm, step_manager_model, step_manager_tok,
                                         sm_device, max_batch=_gpu_batch)
            logger.info(f"vLLM mode: Summary worker → step_manager HF ({sm_device}, path={STEP_MANAGER_PATH})")
        else:
            # single GPU: vLLM으로 요약 (gen과 직렬화 lock 공유)
            ...
            logger.info("vLLM mode: Summary worker → vLLM (single GPU, SFT 공유)")
        ...
        if single_gpu:
            # single GPU: gen과 sum이 같은 모델/GPU 공유
            # → lock으로 직렬화 + 배치를 절반씩 나눠 합계 ≤ batch_per_gpu 유지
            _hf_single_lock = threading.Lock()
            _half_batch = max(1, _gpu_batch // 2)

            def _gen_fn_locked(model, tok, device, states):
                with _hf_single_lock:
                    _batch_run_generator(model, tok, device, states)

            def _sum_fn_locked(model, tok, device, states):
                with _hf_single_lock:
                    _sum_fn(model, tok, device, states)

            sm_device = next(gen_m.parameters()).device
            gen_worker = _GPUBatchWorker(_gen_fn_locked, gen_m, gen_t, gen_d,
                                         max_batch=_half_batch)
            sum_worker = _GPUBatchWorker(_sum_fn_locked, gen_m, gen_t, sm_device,
                                         max_batch=_half_batch)
            logger.info(
                f"HF single-GPU 모드: gen+sum 공유 lock, "
                f"각 max_batch={_half_batch} (batch_per_gpu={_gpu_batch})"
            )
        else:
        ...
        if gen_worker and sum_worker:
            ...
            def _flush_done():
                ...
                while len(active) < n_parallel and queue:
                    active.append(_make_state(queue.pop(0)))
...
def main():
    ...
    parser.add_argument("--fast_rubric_file", type=str, default=None,
                        help="fast 루브릭 JSON 경로 (샘플당 1회 호출). 예: prompts/fast_rubric_v6.0.json")
    ...
    parser.add_argument("--num_end",      type=int, default=None,
                        help="처리 끝 인덱스 (config의 num_end 오버라이드)")
    parser.add_argument("--base_model",    type=str, default=None,
                        help="Generator 체크포인트 경로 (config의 checkpoint.base 오버라이드)")
    parser.add_argument("--base_mode",    action="store_true",
                        help="베이스 모델 평가 모드: 루브릭 없는 기본 프롬프트 사용 (rethink/PRM은 유지)")
    ...
    parser.add_argument("--data_path",    type=str, default=None,
                        help="입력 데이터 파일 경로 (config의 generate_trajectory.base_problems 오버라이드)")
    ...
    # ── CLI 오버라이드 ────────────────────────────────────────────────────────
    if args.base_model:
        CONF["checkpoint"]["base"] = args.base_model
    ...
    elif "base_problems" in gt_cfg:
        dataset_path = gt_cfg["base_problems"]
    else:
        raise KeyError("--data_path 인자 또는 config.generate_trajectory.base_problems 설정이 필요합니다")

    # ── base_mode: generator 프롬프트에서 루브릭 제거 (PRM/rethink는 그대로) ─────────
    if args.base_mode:
        global GEN_SOLVE_PROMPT, GEN_RETHINK_PROMPT, _BASE_MODE
        GEN_SOLVE_PROMPT   = _ACTION_PROMPTS.get("gen_inference_no_rubric",        GEN_SOLVE_PROMPT)
        GEN_RETHINK_PROMPT = _ACTION_PROMPTS.get("gen_rethink_inference_no_rubric", GEN_RETHINK_PROMPT)
        _BASE_MODE = True
        print("[base_mode] generator 프롬프트 루브릭 제거 (PRM/rethink는 기존과 동일)")

    ts          = datetime.now().strftime("%Y%m%d_%H%M%S")
    if args.resume_folder:
        out_dir = Path(args.resume_folder)   # resume 시 같은 폴더에 이어서 저장
    elif args.output:
        out_dir = Path(args.output)
    else:
        out_dir = root / "output" / "sft_trajectory" / ts
    ...
    # ── 출력 파일 (resume 시 append 모드) ────────────────────────────────────
    _file_mode = "a" if args.resume_folder else "w"
    traj_all_file = open(out_dir / "traj_all.jsonl", _file_mode, encoding="utf-8")
    prm_eval_file    = open(prm_out_dir / "prm_evals.jsonl",  _file_mode, encoding="utf-8")
    prm_filter_file  = open(prm_out_dir / "prm_filter.jsonl", _file_mode, encoding="utf-8")
    _run_jsonl_file  = open(out_dir / "run.jsonl", _file_mode, encoding="utf-8")
    ...
    num_end   = args.num_end if args.num_end is not None else gt_cfg.get("num_end")
    ...
    if args.resume_folder:
        ...
        cache_resume_path = Path(args.resume_folder) / "traj_cache.jsonl"
        if cache_resume_path.exists():
            _tmp: dict[str, dict] = {}
            with open(cache_resume_path, encoding="utf-8") as _cf:
                for _line in _cf:
                    _line = _line.strip()
                    if not _line:
                        continue
                    _rec = json.loads(_line)
                    pid = str(_rec.get("problem_id", ""))
                    if pid:
                        _tmp[pid] = _rec
            for pid, rec in _tmp.items():
                if not rec.get("done") and pid not in done_ids and rec.get("all_steps"):
                    partial_cache[pid] = rec
            logger.warning(
                f"[resume cache] {cache_resume_path} 에서 {len(partial_cache)}개 부분 trajectory 로드"
            )

    logger.info(f"로드된 문제 수: {len(items)}")

    # ── traj_cache 셋업 ───────────────────────────────────────────────────────
    # partial_cache로 초기 상태를 설정하고, 스텝마다 전체 재작성 (완료 시 제거)
    _cache_dict: dict = {
        pid: {k: v for k, v in rec.items() if k != "done"}
        for pid, rec in partial_cache.items()
    }
    _cache_file = open(out_dir / "traj_cache.jsonl", "w+", encoding="utf-8")
    for _rec in _cache_dict.values():
        _cache_file.write(json.dumps(_rec, ensure_ascii=False) + "\n")
    _cache_file.flush()
    _cache_lock = __import__("threading").Lock()

    def _save_cache(state):
        pid = str(state.item.get("id", ""))
        with _cache_lock:
            if state.done:
                _cache_dict.pop(pid, None)
            else:
                _cache_dict[pid] = {
                    "problem_id":    pid,
                    "item":          state.item,
                    ...
                    "rethink_round": state.rethink_round,
                }
            _cache_file.seek(0)
            _cache_file.truncate()
            for _r in _cache_dict.values():
                _cache_file.write(json.dumps(_r, ensure_ascii=False) + "\n")
            _cache_file.flush()
    ...
    logger.info(f"GPU 배분: step_manager=cuda:{sm_gpu}(physical {rollout_gpus[0]})  generator={gen_gpus}  single_gpu={single_gpu}")

    # ── Step Manager 로드 (요약·분해 전용) ────────────────────────────────────
    # single_gpu: generator 로드 후 동일 모델을 공유 (별도 로드 생략)
    step_manager_model = None
    step_manager_tok   = None
    if not single_gpu:
        ...
        logger.info("rollout_gpus=1 → Step Manager를 Generator와 공유 (별도 로드 생략)")
    ...
    if USE_VLLM:
        ...
        for i, _gpu in enumerate(gen_gpus):
            # single_gpu: sm=gen=cuda:0 / multi_gpu: sm=cuda:0, gen=cuda:1,2,...
        ...
        if single_gpu and generators:
            ...
            logger.info("rollout_gpus=1 → step_manager_model을 generators[0]와 공유")
    ...
    try:
        generate_batch(
            ...
            step_cache_fn=_save_cache,
            partial_cache=partial_cache,
            step_manager_model=step_manager_model,
            step_manager_tok=step_manager_tok,
        )
    finally:
        traj_all_file.close()
        prm_eval_file.close()
