...
PATCHER API를 활용해 SFT 학습용 step-by-step 추론 데이터 생성
...
출력 형식 (JSONL):
  {
    ...
    "gold_answer": str,
    "is_right":    bool,
    "steps":       [{"step_idx": int, "text": str, "next_gold_action": str}, ...],
...
from utils import (
    CONF, PATCHER, PATCHER_MAX_NEW_TOKENS, SYSTEM_SOLVE_API_SFT,
...
def parse_steps(text: str) -> list[dict]:
    """Step N: 헤더 기준 분리. 없으면 빈 줄 단락 분리 + 불완전 단락 병합."""
    ...
    steps = [{"step_idx": i, "text": t} for i, t in enumerate(texts)]

    # next_gold_action 부여: 마지막 → <|end|>, 나머지 → <|solve|>
    last = len(steps) - 1
    for i, s in enumerate(steps):
        s["next_gold_action"] = "<|end|>" if i == last else "<|solve|>"
...
def solve_problem(item: dict) -> dict | None:
    ...
    messages = [
        {"role": "system", "content": SYSTEM_SOLVE_API_SFT},
...
def main():
    parser = argparse.ArgumentParser(description="SFT 데이터 생성 (step-by-step)")
    ...
    if args.output:
        ...
        output_path = str(out_dir / f"sft_api_{ts}.jsonl")
