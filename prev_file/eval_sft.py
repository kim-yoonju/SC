...
sft 모델을 평가
python source/eval_sft.py 
    --model /mnt/yoonju/SC/checkpoints/sft/20260518_173806_sft/epoch3
    --num_start 7000
    --num_end 7400
    --gpus 0,1
...
출력 (output/eval_gen_solo/{timestamp}/):
  traj_all.jsonl  문제별 결과 (is_right, n_steps, fail_reason, ...)
...
# dual 모드 stage별 max_new_tokens (inference: 추론 1스텝, cls: critique 전체)
INF_MAX_NEW_TOKENS = _GT_CFG.get("inf_max_new_tokens", 2048)
CLS_MAX_NEW_TOKENS = _GT_CFG.get("cls_max_new_tokens", 1024)
...
def _load_prompts() -> tuple[str, str, str, str]:
    """(GEN_SOLVE_PROMPT, GEN_RETHINK_PROMPT, GEN_INF_PROMPT, GEN_CLS_PROMPT) 로드 및 _RUBRIC_NAMES 채우기."""
    ...
    return prompts["gen_solve_R"], prompts["gen_rethink_R"], prompts["gen_inference"], prompts["gen_classification"]


GEN_SOLVE_PROMPT, GEN_RETHINK_PROMPT, GEN_INF_PROMPT, GEN_CLS_PROMPT = _load_prompts()
...
def _traj_summary(state: "ProblemState", is_right: bool, fail_reason: str | None) -> str:
    ...
    for s in state.all_steps:
        pos += 1
        role = s.get("role", "gen")
        action = s.get("next_pred_action") or ""
        if action == "end":
            labels.append(f"G_END")
        elif role == "rethink":
...
def _build_rethink_explanation(state: ProblemState) -> str:
    ...
    fail_details = dict(state.last_wrong_rubric_details)
...
def _batch_run_generator(model, tokenizer, input_device,
    ...
    for j, state in enumerate(states):
        ...
        full_text = full_text + pred_action
...
def _batch_run_generator_vllm(llm, tokenizer, _device,
    ...
    # context length 초과 문제 사전 필터 → 즉시 fail 처리
    try:
        _max_ctx = llm.llm_engine.model_config.max_model_len
    ...
    for i, (state, prompt) in enumerate(zip(states, prompts)):
        ...
        if n_tok >= _max_ctx:
            logger.warning(
                ...
                f"prompt too long ({n_tok} >= {_max_ctx}) → context_overflow"
            )
            state.step_rethink_tried = True
            state.step_substep_tried = True
            state.pending_step = {
                "text":              "[context overflow]",
                "inference":         "[context overflow]",
                "full_text":         "[context overflow]",
                "pred_action":       TOKEN_CORRECT,
                "next_pred_action":  "rethink",
                "gen_fast_critique": {},
                "gen_deep_critique": {},
                "pred_fail_rubrics": [],
                "is_error":          True,
    ...
    if _skip:
        states  = [s for i, s in enumerate(states)  if i not in _skip]
        prompts = [p for i, p in enumerate(prompts) if i not in _skip]
    if not states:
        return
...
# ── Classification 모델용 연속 프롬프트 빌더 ─────────────────────────────────
def _build_cls_continuation_prompt(tokenizer, system: str, user: str, step_text: str) -> str:
    """step_text를 assistant 응답에 포함시킨 분류 모델 프롬프트.
    모델이 step_text 이후 critique 섹션부터만 생성하도록 함.
    """
    messages = [
    ...
    if getattr(tokenizer, "chat_template", None):
        ...
        return base + step_text
    return f"System: {system}\n\nUser: {user}\n\nAssistant: {step_text}"


# ── Generator (Dual: inference + classification) ──────────────────────────────
def _batch_run_generator_dual_hf(
    inf_model, inf_tok, inf_device,
    cls_model, cls_tok, cls_device,
    states: list[ProblemState],
) -> None:
    """Dual-model 2단계 생성.
    Stage 1 – inference_model: gen_inference 프롬프트로 step_text만 생성.
    Stage 2 – classification_model: gen_classification 프롬프트로 critique + pred_action 생성.
    """
    # ── Stage 1: Inference model → step_text 생성 ────────────────────────────
    prompts_inf: list[str] = []
    for state in states:
        lines = [f"[Problem]\n{state.item['problem']}"]
        if state.history:
            lines.append("\n[Previous steps]")
        ...
        lines.append("\nWrite the next step.")
        prompts_inf.append(build_chat_prompt(inf_tok, GEN_INF_PROMPT, "\n".join(lines)))

    orig_side = inf_tok.padding_side
    inf_tok.padding_side = "left"
    enc_inf = inf_tok(prompts_inf, return_tensors="pt", padding=True).to(inf_device)
    inf_tok.padding_side = orig_side
    inf_input_len = enc_inf["input_ids"].shape[1]

    with torch.no_grad():
        out_inf = inf_model.generate(
            **enc_inf,
    ...
    resp_inf_all = out_inf[:, inf_input_len:]
    step_texts: list[str] = []
    for j in range(len(states)):
        inf_text = inf_tok.decode(resp_inf_all[j], skip_special_tokens=False).strip()
        step_text, _ = _split_step_and_critique(inf_text)
        step_texts.append(step_text)

    # free inference model GPU memory before classification stage
    del out_inf, enc_inf, resp_inf_all
    torch.cuda.empty_cache()

    # ── Stage 2: Classification model → critique + pred_action 생성 ──────────
    # gen_classification 포맷: [Current step]\n{step_text}\n\nEvaluate this step.
    prompts_cls: list[str] = []
    for state, step_text in zip(states, step_texts):
        lines = [f"[Problem]\n{state.item['problem']}"]
        if state.history:
            lines.append("\n[Previous steps]")
            for s in state.history:
        ...
        lines.append(f"\n[Current step]\n{step_text}")
        lines.append("\nEvaluate this step.")
        prompts_cls.append(build_chat_prompt(cls_tok, GEN_CLS_PROMPT, "\n".join(lines)))

    _action_ids = [
        cls_tok.convert_tokens_to_ids(t)
        for t in [TOKEN_SOLVE, TOKEN_CORRECT, TOKEN_END]
    ]
    _action_ids = [tid for tid in _action_ids if tid is not None and tid != cls_tok.unk_token_id]
    _eos_ids    = [tid for tid in {cls_tok.eos_token_id, *_action_ids} if tid is not None]

    orig_side = cls_tok.padding_side
    cls_tok.padding_side = "left"
    enc_cls = cls_tok(prompts_cls, return_tensors="pt", padding=True).to(cls_device)
    cls_tok.padding_side = orig_side
    cls_input_len = enc_cls["input_ids"].shape[1]

    from transformers import LogitsProcessorList
    _fr_allowed, _fr_next = _get_fail_rubrics_allowed_ids(cls_tok)
    _fr_processor = _FailRubricsLogitsProcessor(cls_input_len, cls_tok, _fr_allowed, _fr_next)
    _na_processor = _NextActionHFLogitsProcessor(cls_input_len, cls_tok, _action_ids)

    with torch.no_grad():
        out_cls = cls_model.generate(
            **enc_cls,
            max_new_tokens=CLS_MAX_NEW_TOKENS,
            do_sample=False,
            pad_token_id=cls_tok.eos_token_id,
            eos_token_id=_eos_ids,
            logits_processor=LogitsProcessorList([_fr_processor, _na_processor]),
        )

    im_end_id   = cls_tok.convert_tokens_to_ids("<|im_end|>")
    _stop_ids   = {cls_tok.pad_token_id, im_end_id, *_action_ids}
    _id_to_action = {
        cls_tok.convert_tokens_to_ids(TOKEN_SOLVE):   TOKEN_SOLVE,
        cls_tok.convert_tokens_to_ids(TOKEN_CORRECT): TOKEN_CORRECT,
        cls_tok.convert_tokens_to_ids(TOKEN_END):     TOKEN_END,
    }

    resp_cls_all = out_cls[:, cls_input_len:]
    ...
    for j, state in enumerate(states):
        ...
        cls_resp = resp_cls_all[j]
        trim = cls_resp.shape[0]
        stopped_action = None
        for pos, tid in enumerate(cls_resp.tolist()):
            if tid in _stop_ids:
                trim = pos
                stopped_action = _id_to_action.get(tid)
                break
        # cls 모델이 생성한 critique 섹션 (step_text 이후부터)
        cls_generated = cls_tok.decode(cls_resp[:trim], skip_special_tokens=False).strip()
        # 파싱 함수용: step_text + critique 결합
        cls_full_text = step_text + ("\n" + cls_generated if cls_generated else "")
        # self_check_text: leading \n 보장해 critique marker 탐지 정확도 유지
        self_check_text = (
            "\n" + cls_generated
            if cls_generated and not cls_generated.startswith("\n")
            else cls_generated
        )
        ...
        full_text = step_text + (self_check_text if self_check_text else "")

        if pred_action == TOKEN_END and not has_boxed(full_text):
            logger.info(
                f"[Dual-HF] id={state.item.get('id')} step={len(state.history)+1} "
                f"TOKEN_END without \\boxed{{}} → override to TOKEN_SOLVE"
            )
            pred_action = TOKEN_SOLVE
        ...
        logger.info(
            f"[Dual-HF] id={state.item.get('id')} "
            f"step={len(state.history)+1} rethink={state.is_rethink} "
            f"pred_action={pred_action} is_error={is_error}"
        )
        ...
        state.pending_step = {
            ...
            "pred_action":       pred_action,
            "next_pred_action":  _action_to_str(pred_action),
            "gen_fast_critique": _parse_gen_fast_critique(cls_full_text, _RUBRIC_NAMES),
            "gen_deep_critique": _parse_generator_deep_check(cls_full_text, _RUBRIC_NAMES),
            "pred_fail_rubrics": _parse_pred_fail_rubrics_from_text(cls_full_text),
            "is_error":          is_error,
            "role":              "rethink" if state.is_rethink else "gen",
        }
...
def _print_step_verbose(state: "ProblemState", step: dict) -> None:
    ...
    if action == "<|rethink|>":
        ...
        action_r = 1.0 if has_boxed else 0.0
...
def _process_gen_result(state: ProblemState, save_fn=None,
    ...
    if raw_step_save_fn:
        raw_step_save_fn({
            ...
            "text":              step.get("full_text", step["text"]),
            "inference":         step["text"],
    ...
    def _finish(is_right: bool, fail_reason: str = None, pred_answer: str = None):
        result = {
            ...
            "steps": [
                ...
                for i, s in enumerate(state.all_steps)
            ],
        }
        state.result = result
        if save_fn:
...
def _parallel_gen_dual(generators: list, classifiers: list, states: list) -> None:
    """(inf, cls) 쌍 여러 개에 문제를 분배해 병렬로 dual-HF 생성."""
    n      = len(generators)
    splits = [states[i::n] for i in range(n)]

    def _run(i):
        if splits[i]:
            inf_model, inf_tok, inf_device = generators[i]
            cls_model, cls_tok, cls_device = classifiers[i]
            _batch_run_generator_dual_hf(
                inf_model, inf_tok, inf_device,
                cls_model, cls_tok, cls_device,
                splits[i],
            )
...
def evaluate_gen_batch(
    ...
    while active:
        ...
        _round_idx += 1
        tqdm.write(f"[Round {_round_idx}] active={len(active)}  done={pbar.n}/{pbar.total}")
        logger.info(f"[Round {_round_idx}] active={len(active)}")

        # 생성
        if classifiers and generators:
            # dual 모드: inference_model → step_text, classification_model → pred_action
            if len(generators) > 1:
                _parallel_gen_dual(generators, classifiers, active)
            else:
                inf_model, inf_tok, inf_device = generators[0]
                cls_model, cls_tok, cls_device = classifiers[0]
                _batch_run_generator_dual_hf(
                    inf_model, inf_tok, inf_device,
                    cls_model, cls_tok, cls_device,
                    active,
...
def main():
    ...
    parser.add_argument("--num_start",  type=int, default=None,
                        help="시작 인덱스 (기본: inference.num_start or 0)")
    parser.add_argument("--num_end",    type=int, default=None,
                        help="종료 인덱스(exclusive) (기본: inference.num_end or 전체)")
    parser.add_argument("--model",      type=str, default=None,
                        help="사용할 모델 경로 (기본: checkpoint.sft_checkpoint or checkpoint.base)")
    ...
    parser.add_argument("--gpus", type=str, default=None,
                        help="사용할 GPU 번호 (예: '3' 또는 '3,4,5'). 기본: config의 inference.gpus")
    parser.add_argument("--inference_model", type=str, default=None,
                        help="추론용 모델 경로 (dual 모드). --classification_model과 함께 사용 시 "
                             "이 모델이 step_text를 생성하고, classification_model이 next action을 예측.")
    parser.add_argument("--classification_model", type=str, default=None,
                        help="next action 예측용 모델 경로 (dual 모드). --inference_model과 함께 사용.")
    ...
    if args.debug:
        ...
        num_start = args.num_start if args.num_start is not None else (inf_cfg.get("num_start") or 0)
        num_end   = args.num_end   if args.num_end   is not None else inf_cfg.get("num_end")
    ...
    if args.gpus:
        rollout_gpus = [int(g) for g in args.gpus.split(",")]
    else:
        rollout_gpus = inf_cfg.get("gpus") or gt_cfg.get("rollout_gpus", [0])
    ...
    step_manager_model  = None
    step_manager_tok    = None
    _dual_mode = bool(args.inference_model and args.classification_model)

    if not _GENERATOR_API and not single_gpu and not _dual_mode:
        # GPU 2개 이상, 단일 모드 → 첫 번째 GPU에 step_manager 별도 로드
        ...
        logger.info("rollout_gpus=1 또는 API 모드 또는 dual 모드 → Step Manager 별도 로드 생략")

    # ── Generator 로드 ────────────────────────────────────────────────────────
    if _dual_mode and not _GENERATOR_API:
        # Dual 모드: inference_model(추론) + classification_model(critique/action 예측)
        cls_model_id = args.classification_model
        inf_model_id = args.inference_model

        if single_gpu:
            # GPU 1개: 두 모델 공유 → inference model만 올리고 cls 역할도 수행
            inf_dmap = {"": f"cuda:{rollout_gpus[0]}"}
            logger.info(f"Inference 모델 로딩 (single GPU): {inf_model_id}  device={inf_dmap}")
            model, tokenizer = load_generator(model_path=inf_model_id, device_map=inf_dmap)
            generators.append((model, tokenizer, next(model.parameters()).device))
            cls_model, cls_tok = load_generator(model_path=cls_model_id, device_map=inf_dmap)
            classifiers.append((cls_model, cls_tok, next(cls_model.parameters()).device))
        else:
            # GPU 2개씩 쌍으로 (inf_gpu, cls_gpu) 구성. 쌍 수 = len(rollout_gpus) // 2
            pair_count = len(rollout_gpus) // 2
            for p in range(pair_count):
                inf_gpu  = rollout_gpus[p * 2]
                cls_gpu  = rollout_gpus[p * 2 + 1]
                inf_dmap = {"": f"cuda:{inf_gpu}"}
                cls_dmap = {"": f"cuda:{cls_gpu}"}

                logger.info(f"[쌍 {p+1}/{pair_count}] Inference 모델 로딩: {inf_model_id}  device=cuda:{inf_gpu}")
                model, tokenizer = load_generator(model_path=inf_model_id, device_map=inf_dmap)
                generators.append((model, tokenizer, next(model.parameters()).device))

                logger.info(f"[쌍 {p+1}/{pair_count}] Classification 모델 로딩: {cls_model_id}  device=cuda:{cls_gpu}")
                cls_model, cls_tok = load_generator(model_path=cls_model_id, device_map=cls_dmap)
                classifiers.append((cls_model, cls_tok, next(cls_model.parameters()).device))

            logger.info(f"Dual 모델 {pair_count}쌍 로드 완료")

        if single_gpu and generators:
            step_manager_model = generators[0][0]
            step_manager_tok   = generators[0][1]
            logger.info("rollout_gpus=1(dual) → step_manager를 inference generators[0]와 공유")
    ...
    run_meta = {
        ...
        "mode":        "api" if _GENERATOR_API else ("vllm" if USE_VLLM else ("dual-hf" if _dual_mode else "hf")),
        **({"inference_model": args.inference_model, "classification_model": args.classification_model}
           if _dual_mode else {}),
    }
    ...
    def _save(rec: dict):
        ...
        traj_file.write(json.dumps(rec, ensure_ascii=False) + "\n")
        traj_file.flush()
        if not _first_printed and rec.get("problem_id") == str(items[0].get("id", "?")):
            steps = rec.get("steps", [])
            if steps:
                first = steps[0]
                npa   = first.get("next_pred_action") or ""
                print(f"\n[Generator 출력 - Step 1]\n{first.get('text', '')}\n→ next_action: {npa}\n")
            _first_printed = True
    ...
    action_counts = {"solve": 0, "rethink": 0, "end": 0}
    total_steps = 0
    for r in results:
        for s in r.get("steps", []):
            npa = s.get("next_pred_action") or ""
            if npa in action_counts:
                action_counts[npa] += 1
                total_steps += 1
    ...
    print(f"스텝 action 분포 (총 {total_steps}스텝):")
    for act, cnt in action_counts.items():
        ratio = cnt / total_steps if total_steps else 0
        print(f"  {act:8s}: {cnt:5d} ({ratio:.1%})")
    ...
    summary = {
        ...
        "action_ratios":  {k: round(v / total_steps, 4) if total_steps else 0 for k, v in action_counts.items()},
